...
#ifdef WIN32
typedef HANDLE pthread_snafu_t;

inline pthread_snafu_t CreateThread(void(*pfn)(void*), void* parg, bool fWantHandle=false)
{
    DWORD nUnused = 0;
    HANDLE hthread =
        CreateThread(
    ...
    if (hthread == NULL)
    {
        ...
        return (pthread_snafu_t)0;
    }
    if (!fWantHandle)
    {
        CloseHandle(hthread);
        return (pthread_snafu_t)-1;
    }
    return hthread;
}
